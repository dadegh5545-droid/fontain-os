...
العقد المحايد بين الطرفين: `enums.ts` (القيم والأنواع)، `enum-labels.ts` (ترجمة ar/en لكل قيمة)، `phone.ts` (تطبيع E.164، يستخدمه الـOTP في FOS-005)، ثم `contracts/` و`validators/` لاحقًا، ويُستورد من الواجهة والخلفية معًا عبر `@shared/*`.
